...
과제 안내문을 읽고 제가 가장 중요하게 본 지점은, RAG가 통제된 데모 환경에서는 쉽게 동작해 보일 수 있지만 실제 서비스 환경에서는 문서 범위 밖 질문, 모호한 질문, 검색 실패, 근거 부족, citation 누락 같은 edge case에서 품질이 크게 흔들릴 수 있다는 점이었습니다.

따라서 이번 구현에서는 단순히 답변을 생성하는 것보다 다음 세 가지를 우선순위로 두었습니다.
